...
def get_sqlalchemy_engine():
    ...
    try:
        ...
        database_url = f"postgresql://alex_orator:{encoded_password}@localhost:5434/app_db"
...
def authenticate_user(username, password):
    ...
    try:
        ...
        if engine:
            ...
            with engine.connect() as conn:
                ...
                if admin_table_exists:
                    ...
                    if row:
                        # Проверяем пароль (используем простое хеширование SHA256)
        ...
        # Если пользователь не найден в БД, проверяем системного администратора
        
        is_valid = username in valid_users and valid_users[username] == password
        
        if is_valid:
            logger.info(f'Успешная авторизация системного администратора: {username}')
        else:
        ...
        # Fallback к системному администратору при ошибке БД
        is_valid = username in valid_users and valid_users[username] == password
        
        if is_valid:
            logger.info(f'Успешная авторизация системного администратора {username} (fallback)')
        else:
            logger.warning(f'Неудачная попытка авторизации для пользователя {username} (fallback)')
...
def get_user_role(username):
    ...
    try:
        ...
        # Fallback к системной роли при ошибке БД
        user_roles = {
            "admin": "Системный администратор"
        }
        return user_roles.get(username, "Неизвестно")
...
def get_user_info(username):
    ...
    try:
        ...
        # Fallback к системному администратору
