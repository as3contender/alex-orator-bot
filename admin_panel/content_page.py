        ...
        if content_list:
            ...
            if filtered_content:
                ...
                for item in filtered_content:
                    ...
                    content_preview = item['content_text'][:30] + "..." if len(item['content_text']) > 30 else item['content_text']
                ...
                with st.form("edit_content_form"):
                    ...
                    with col1:
                        if st.form_submit_button("💾 Сохранить", type="primary"):
                            if new_text and new_text != selected_item['content_text']:
                                if db.update_bot_content(selected_item['content_key'], new_text, selected_item['language']):
                    ...
                    with col3:
                        if st.form_submit_button("🗑️ Удалить навсегда", type="secondary"):
                            ...
                            if st.checkbox("Подтверждаю полное удаление", key="confirm_permanent_delete"):
                                if db.permanently_delete_bot_content(selected_item['content_key'], selected_item['language']):
...
def content_stats_tab():
    ...
    try:
        ...
        try:
            ...
            sql_results = db.execute_sql_file(os.path.join('backend', 'services', 'statistics.sql'))
            ...
            if sql_results:
                ...
                df_sql.columns = ['Дата', 'Новые пользователи', 'Регистрации на неделю', 'Новые пары', 'Подтвержденные пары']
    ...
    try:
        ...
        for item in content_list:
            export_data.append({
                ...
                "Дата обновления": item['updated_at'].strftime("%Y-%m-%d %H:%M:%S") if item['updated_at'] else "N/A",
        ...
        df_export.to_csv(csv_buffer, index=False, encoding='utf-8-sig')  # utf-8-sig для корректного отображения кириллицы в Excel
